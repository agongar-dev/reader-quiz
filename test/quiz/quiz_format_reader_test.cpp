...
class FileQuizInput : public QuizInput {
 public:
  explicit FileQuizInput(const std::string& path) : file_(path, std::ios::binary | std::ios::ate) {
    if (file_) size_ = static_cast<uint64_t>(file_.tellg());
  }
...
template <typename T>
std::string_view view(const T& bytes, size_t len) {
  return {bytes.data(), len};
}

TEST(QuizFormatReader, LoadsValidDeckAndQuestionsByOrdinal) {
  FileQuizInput input(fixturePath("valid.quiz"));
  QuizFormatReader reader(input);
  QuizDeckInfo deck;
  ASSERT_EQ(reader.validate(&deck), QuizFormatError::Ok);
  EXPECT_EQ(view(deck.title, deck.titleBytes), "Mini Quiz");
  EXPECT_EQ(deck.questionCount, 2u);
  EXPECT_NE(deck.deckId[0], 0);
  EXPECT_NE(deck.revisionId[0], 0);

  QuizQuestionData q;
  ASSERT_EQ(reader.loadQuestion(deck, 1, &q), QuizFormatError::Ok);
  EXPECT_EQ(view(q.prompt, q.promptBytes), "Pi?");
  EXPECT_EQ(q.choiceCount, 3);
  EXPECT_EQ(q.correctChoiceIndex, 1);
  EXPECT_EQ(view(q.choices[0], q.choiceBytes[0]), "2");
  EXPECT_EQ(view(q.choices[1], q.choiceBytes[1]), "3.14");
  EXPECT_EQ(view(q.choices[2], q.choiceBytes[2]), "4");
  EXPECT_EQ(view(q.explanation, q.explanationBytes), "Approx.");
  ASSERT_EQ(reader.loadQuestion(deck, 0, &q), QuizFormatError::Ok);
  EXPECT_EQ(view(q.prompt, q.promptBytes), "First?");
  EXPECT_EQ(q.choiceCount, 2);
  EXPECT_EQ(q.correctChoiceIndex, 1);
...
TEST_P(InvalidFixtureTest, RejectsInvalidDecks) {
  ...
  FileQuizInput input(fixturePath(name));
  QuizFormatReader reader(input);
  QuizDeckInfo deck;
  EXPECT_EQ(reader.validate(&deck), expected) << name;
}
INSTANTIATE_TEST_SUITE_P(
    InvalidFixtures, InvalidFixtureTest,
